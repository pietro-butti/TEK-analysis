...
basement = 'n361b0340k7hf1875'
ciao = np.loadtxt('DATA/PROCESSED/'+basement+'_creutz.dat').T
...
for R in [int(sys.argv[1])]:
    ...
    if len(sys.argv)==4:
        ...
        fit,errfit = curve_fit(chin,x,y,sigma=erry,p0=[0.1,30,0.1])
        ...
        plt.axvline(ciao[0][minfit],color='gray',alpha=0.7)
        plt.axvline(ciao[0][maxfit],color='gray',alpha=0.7)

        plt.grid(zorder=0, linewidth=0.5, color='gainsboro')
        plt.legend()
        
        if input("Wanna save? (y  or n): ")=='y': plt.savefig('PLOTS/'+basement+'_chi('+str(R)+',5)')
        plt.show()
